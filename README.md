...
- **输入与控制增强** — 支持以 `CoreHID` 为核心的低延迟、高回报率鼠标输入链路，带来更直接、更细腻的相对移动响应；自由鼠标模式可无缝切换到其他屏幕继续操作，锁定鼠标模式更适合游戏与持续相对移动场景，并提供可自定义串流快捷键、手柄增强，以及 Foundation Sunshine 下的双向剪贴板文本 / 单张图片同步
